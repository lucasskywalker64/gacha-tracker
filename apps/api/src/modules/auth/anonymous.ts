...
export const anonymousAuthPlugin = new Elysia({ name: "anonymous-auth" })
   ...
   */
  .post("/api/auth/anonymous/generate", async ({ status }) => {
   ...
   */
  .post("/api/auth/anonymous/confirm", async ({ body, request, status }) => {
    const { code } = body;
    const codeHash = await hashCode(code);
    const redisKey = `anon_pending:${codeHash}`;
   ...
   */
  .post("/api/auth/anonymous/login", async ({ body, request, status }) => {
    const { code } = body;
