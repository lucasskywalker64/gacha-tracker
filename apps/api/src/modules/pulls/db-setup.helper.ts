...
export async function createTestTables(sqlite: Client) {
    ...
    await sqlite.execute(
        `CREATE TABLE IF NOT EXISTS import_log (id TEXT PRIMARY KEY, user_id TEXT NOT NULL, game_id TEXT, game_uid TEXT, initiated_at INTEGER NOT NULL, completed_at INTEGER, status TEXT NOT NULL DEFAULT 'pending', total_fetched INTEGER, new_pulls INTEGER, duplicates INTEGER, error_message TEXT, source_ip TEXT, script_duration_ms INTEGER, pity_calc_duration_ms INTEGER, db_write_duration_ms INTEGER, backend_duration_ms INTEGER, queue_wait_duration_ms INTEGER, total_duration_ms INTEGER, import_method TEXT, script_version TEXT, web_app_version TEXT, file_version TEXT, user_agent TEXT, payload_size_bytes INTEGER, earliest_pull_at INTEGER, latest_pull_at INTEGER, banners_affected_count INTEGER, error_code TEXT, raw_error_stack TEXT)`
    );
