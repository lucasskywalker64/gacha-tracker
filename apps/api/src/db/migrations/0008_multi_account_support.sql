...
ALTER TABLE `user_game` ADD `game_uid` text NOT NULL;--> statement-breakpoint
ALTER TABLE `user_game` ADD `nickname` text;--> statement-breakpoint
ALTER TABLE `user_game` ADD `is_primary` integer DEFAULT false NOT NULL;--> statement-breakpoint
CREATE UNIQUE INDEX `user_game_user_game_uid_idx` ON `user_game` (`user_id`,`game_id`,`game_uid`);--> statement-breakpoint
CREATE UNIQUE INDEX `idx_user_game_primary` ON `user_game` (`user_id`,`game_id`) WHERE "user_game"."is_primary" = 1;--> statement-breakpoint
