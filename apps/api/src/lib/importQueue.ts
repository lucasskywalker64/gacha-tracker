...
async function persistTaskToRedis(entry: QueueEntry) {
    try {
        ...
        if (isTerminal) {
            ...
            await redis.del(`import_task_buffer:${entry.requestId}`);
            await redis.srem("import_tasks_active", entry.requestId);
        } else {
            await redis.expire(key, 86400); // 24h fallback TTL for active tasks
            await redis.sadd("import_tasks_active", entry.requestId);
            ...
            if (entry.buffer && entry.buffer.length > 0) {
                ...
                await redis.set(bufKey, entry.buffer.toString("base64"), "EX", 86400);
            }
        }
    } catch {
